...
void Svpwm_lay(SvpwmStr_t* Str_p, DynaSinTableStr_t* DynaStr_p, uint16_t* Pwm_p, double* Theta_p) {
    ...
    for (int c = 0; c < DynaStr_p->Channel; c++) {
        SvpwmSinTable_TwoCos_p[c] = (int32_t)(cos(Str_p->Omega) * (1 << 15) *
            (1 << 15));    // F3.13= (int16_t)(2^13*(double -2~2))
        for (int j = 0; j < 3; j++) {
            SvpwmSinTable_S0_p[c * 3 + j] =
                (int32_t)(sin(Str_p->Theta_p[c] + (j - 1) * Str_p->Omega) * (1 << 15) *
                    (1 << 15));    // F2.14=(int16_t)(2^14*(double -1~1))
        }
    }
}
...
uint8_t Svpwm_step(void* void_p) {
    ...
    for (uint8_t i = 0; i < Str_p->Channel; i++)
        Str_p->Pwm_p[i] = (*Str_p->CurrentIn_p * (0x01 << 14 + Str_p->DynaStr_p->s_p[i])) >> 2;
    return 0;
}
